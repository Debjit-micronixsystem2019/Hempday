...
class DispensariesMapVC: UIViewController,CLLocationManagerDelegate,MKMapViewDelegate {
    ...
    func createAnnotations(location: [[String : Any]]){
        ...
        let center = CLLocationCoordinate2D(latitude: 36.114647, longitude: -115.172813)
...
/*    let span = MKCoordinateSpan(latitudeDelta: 36.114647, longitudeDelta: -115.172813)
    ...
    mapView.addAnnotation(annotation)
        ...
        pointAnnotation.pinCustomImageName = "9"
        pointAnnotation.coordinate = locValue
...
/*extension DispensariesMapVC : UISearchBarDelegate{
   
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if DispensariecListArray.count != 0{
            NonFilterArray = lasveagsAreaArray.filter({$0.name.prefix(searchText.count) == searchText.uppercased()})
